...
def main():
    ...
    parser.add_argument('-b', '--band', type=int, nargs='+', required=True, help='Band index (e.g. -1 1 2)')
    ...
    if vec_file is None:
        for suffix in ['2d', '2D', 'chern', 'Chern', 'CHERN', '']:  # 兼容不同后缀
            ...
            if os.path.exists(candidate):
                ...
                break
    if vec_file is None:
        print(f"[ERROR] 波函数文件未找到，请检查 {output_dir} 下的 vec_{valley_str}_valley_2d.npy 等文件是否存在。")
        sys.exit(1)
    ...
    fig_suffix = f'{valley_str}_{num_chern}'
    for band_index in args.band:
        ...
        berry_img = os.path.join(output_dir, f'berry_band{band_index}_{fig_suffix}.pdf')
        ...
        with open(os.path.join(output_dir, f'chern_band{band_index}.txt'), 'w') as f:
            f.write(f'Chern number for band {band_index}: {chern_number:.6f}\n')
        ...
        qmetric_img = os.path.join(output_dir, f'qmetric_band{band_index}_{fig_suffix}.pdf')
